export default function About() {
  return (
    <>
      <div className="font-bold mt-6 text-xl"> About Me</div>
      <div className="mt-5"><span className="ml-10"></span>
        Hello, Myself Arnab Pal, I am a Passionate Frontend Developer, Who is
        currently focused on learning Backend Development. My Passion for Web
        Dev Started in My Early Years of college life When I used to Make Simple
        HTML and transitioning those through beautiful designs. Throughout my
        journey in Tech, I also have the privilege of working with Artificial
        Intelligence and utilized it to enhance performance of various apps.
      </div>
      <div className="mt-5"><span className="ml-10"></span>
        Although in this 1.5 years of journey,
        While I may not be a widely recognized figure, i am readily available for connection on my social media platforms.
      </div>
    </>
  );
}
